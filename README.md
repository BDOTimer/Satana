...
Если желаете получить последовательность числа ПИ содержащую все вхождения восьмизначных чисел **БЕЗ хака**, то для это нужно `AMOUNT_ITERATIONS = 29` и конфигурацию компа с `памятью 32 гб.`.    
